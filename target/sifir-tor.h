 ...
 */
struct BoxedResult_TcpSocksStream *tcp_stream_start(const char *target, const char *proxy,
                                                    uint64_t timeout_ms);
 ...
 * Send a Message over a tcpStream
 */
struct ResultMessage *tcp_stream_send_msg(TcpSocksStream *stream_ptr, const char *msg,
                                          uint64_t timeout);
...
struct BoxedResult_____c_char *create_hidden_service(OwnedTorService *owned_client,
                                                     uint16_t dst_port, uint16_t hs_port,
 ...
 */
struct BoxedResult_HiddenServiceHandler *
start_http_hidden_service_handler(uint16_t dst_port, struct Observer observer);
